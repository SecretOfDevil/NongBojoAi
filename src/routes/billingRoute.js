...
router.get("/offer/gpt5", requireApiKey, async (req,res,next) => { try { const claim=await getOfferStatus(req.apiKey,"gpt5-100k"); res.json({ eligible:["free","plus"].includes(req.keyRecord.tier) && !claim, claim, model:"openai/gpt-5.6-terra", tokens:100000 }); } catch(e){next(e)} });
